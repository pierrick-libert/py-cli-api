...
```bash
python main.py create -t sport -d '{"name": "Football", "display_name": "Football", "order": 0, "is_active": true}'
```

#### Update

```bash
python main.py update --type sport --data '{"is_active": false}' --id 'sport_uuid'
```

#### Delete

```bash
python main.py delete --type sport --id 'sport_uuid'
```

#### Search

```bash
python main.py search --type sport --data '[{"field": "name", "operator": "in", "value": ["Football", "Test"]}, {"field": "is_active", "operator": "=", "value": false}]'
```
...
```bash
python main.py create -t event -d '{"sport_id": "sport_uuid", "name": "Football v England", "display_name": "Football v England", "type": "inplay", "status": "inplay", "is_active": true}'
```

#### Update

```bash
python main.py update --type event --data '{"is_active": false}' --id 'event_uuid'
```

#### Delete

```bash
python main.py delete --type event --id 'event_uuid'
```

#### Search

```bash
python main.py search --type event --data '[{"field": "type", "operator": "notin", "value": ["PREPLAY"]}]'
```
...
```bash
python main.py create -t market -d '{"event_id": "event_uuid", "name": "Full Time Result", "display_name": "Full Time Result", "order": 0, "schema": 1, "columns": 5, "is_active": true}'
```

#### Update

```bash
python main.py update --type market --data '{"is_active": false}' --id 'market_uuid'
```

#### Delete

```bash
python main.py delete --type market --id 'market_uuid'
```

#### Search

```bash
python main.py search --type market --data '[{"field": "columns", "operator": ">=", "value": 5}]'
```
...
```bash
python main.py create -t selection -d '{"market_id": "market_uuid", "name": "Full Time Result", "display_name": "Full Time Result", "price": 10.01, "outcome": "UNSETTLED", "is_active": true}'
```

#### Update

```bash
python main.py update --type selection --data '{"is_active": false}' --id 'selection_uuid'
```

#### Delete

```bash
python main.py delete --type selection --id 'selection_uuid'
```

#### Search

```bash
python main.py search --type selection --data '[{"field": "name", "operator": "regex", "value": "Full Time Result"}]'
```
